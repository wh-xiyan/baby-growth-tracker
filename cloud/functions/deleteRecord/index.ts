...
export async function main(event: { familyId?: string; recordId?: string }) {
  ...
  if (!event.recordId) throw new Error('INVALID_RECORD'); await db.collection('growth_records').doc(event.recordId).update({ data: { deletedAt: now(), updatedAt: now() } }); return result({ id: event.recordId })
}
