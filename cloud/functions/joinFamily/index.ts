...
export async function main(event: { inviteCode?: string; displayName?: string }) {
  ...
  const family = await db.collection('families').where({ inviteCode: code }).limit(1).get()
  if (!family.data[0]) throw new Error('FAMILY_NOT_FOUND')
  const existing = await db
    .collection('family_members')
    .where({ familyId: family.data[0]._id, userId: user.data[0]._id })
    .limit(1)
    .get()
  ...
  const data = {
    ...
    role: 'adult',
    displayName: event.displayName?.trim() || user.data[0].nickname || '家庭成员',
    status: 'active',
    joinedAt: now(),
    updatedAt: now(),
  }
  if (existing.data[0])
    await db.collection('family_members').doc(existing.data[0]._id).update({ data })
