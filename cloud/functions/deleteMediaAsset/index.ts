...
export async function main(event: { familyId?: string; mediaId?: string }) {
  ...
  if (!event.mediaId) throw new Error('INVALID_MEDIA'); await db.collection('media_assets').doc(event.mediaId).update({ data: { deletedAt: now() } }); return result({ id: event.mediaId })
}
