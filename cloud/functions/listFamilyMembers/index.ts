...
export async function main(event: { familyId?: string }) {
  ...
  const members = await db.collection('family_members').where({ familyId, status: 'active' }).get()
  return result(members.data.map((item: Record<string, any>) => ({ ...item, id: item._id })))
}
