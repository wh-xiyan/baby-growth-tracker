...
export async function main(event: { familyId?: string; childId?: string }) {
  ...
  const assets = await db.collection('media_assets').where(where).orderBy('createdAt', 'desc').get()
  return result(assets.data.map((item: Record<string, any>) => ({ ...item, id: item._id })))
}
